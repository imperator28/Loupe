...
- Unit review now has a tested end-to-end correction path: a persisted, source-identity-bound `mm`/`in` choice is submitted through the worker protocol and reimports geometry analysis at the selected scale. It intentionally bypasses unqualified cache replay so an old scale cannot be presented as reviewed. Custom multipliers, source-change warning UI, and export blocking evidence remain open.
- Gate C now has geometry-backed component measurements for area, volume, bounds, longest-edge, and circular-feature radius/diameter; renderer hit-based component selection, point-to-point, surface-distance, and normal-angle measurements; CPU triangle clipping for X/Y/Z and selected-face planes; and planar cap/slice triangulation from the clipped mesh. The cap and 2D-slice controls are presentation-only and capture can include or temporarily omit cap geometry. PNG capture uses a Qt item grab at the resolved 1×–4× viewport size and can include the active measurement overlay. It is **not complete**: exact individual OCCT edge/face picking (rather than component topology summaries), real-GPU capture validation, and corpus/performance evidence remain open. These are correctness requirements, not polish.
- Gate D remains open. `scripts/run-loupe-debug.sh` now mirrors the Windows developer launcher for Apple Silicon, but it must be exercised on the M2 Pro. Do not record P1 as macOS-ready until the remaining Gate C precision work and benchmark/evidence work are complete.
