...
- Gate C now has geometry-backed component measurements for area, volume, bounds, longest-edge, and circular-feature radius/diameter; renderer hit-based component selection and point-to-point measurements; CPU triangle clipping in the inspect viewer for X/Y/Z section planes and flip; and a Qt item-grab PNG save path at the resolved 1×–4× viewport size. It is **not complete**: precise individual topology picking (edges/faces), angle measurement, section caps/planar-face planes/2D slice presentation, and capture inclusion of measurement overlays or section caps remain open. The worker's progressive snapshot/mesh stream is now connected to the shell, but real GPU capture/rendering on representative hardware remains to be evidenced. These are correctness requirements, not polish.
