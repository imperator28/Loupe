...
- Gate C now has geometry-backed component measurements for area, volume, and bounds, CPU triangle clipping in the inspect viewer for X/Y/Z section planes and flip, and a Qt item-grab PNG save path at the resolved 1×–4× viewport size. It is **not complete**: individual topology picking (points, edges, faces, radii, angles), section caps/planar-face planes/2D slice presentation, and capture inclusion of measurement overlays or section caps remain open. The worker's progressive snapshot/mesh stream is now connected to the shell, but real GPU capture/rendering on representative hardware remains to be evidenced. These are correctness requirements, not polish.
