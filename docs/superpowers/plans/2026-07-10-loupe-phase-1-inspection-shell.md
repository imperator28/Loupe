...
- Gate C now has geometry-backed component measurements for area, volume, and bounds, plus CPU triangle clipping in the inspect viewer for X/Y/Z section planes and flip. It is **not complete**: individual topology picking (points, edges, faces, radii, angles), section caps/planar-face planes/2D slice presentation, and a platform offscreen PNG capture adapter remain open. The worker's progressive snapshot/mesh stream is now connected to the shell, but rendering on representative hardware remains to be evidenced. These are correctness requirements, not polish.
